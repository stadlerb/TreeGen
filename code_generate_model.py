...
class code_gen_model:
    ...
    def nl_reader(self, state, embedding_size, step, halting_probability, remainders, n_updates, previous_state,
        ...
        state += self.get_timing_signal_1d(tf.shape(state)[1], tf.shape(state)[2],
                                           start_index=0) + self.get_timing_signal_1d(tf.shape(state)[1],
                                                                                      tf.shape(state)[2],
    ...
    def ast_reader(self, state, embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                   Decoder1, nl_conv, state_ast, ):
        size = embedding_size
        i = step
        state_shape_static = state.get_shape()
    ...
    def transf(self, em_NL, mask, layers, position=False, size=None, em_char=""):
        ...
        for i in range(int(self.max_steps)):
            with tf.variable_scope("NL_CONV" + str(i), reuse=None):
                (state, size, step, halting_probability, remainders, n_updates, previous_state, em_char, mask,
                 mask) = self.nl_reader(state, size, step, halting_probability, remainders, n_updates, previous_state,
                                        em_char, mask, mask)
        # self.probe = state
        return state
    ...
    def __init__(self, classnum, embedding_size, conv_layernum, conv_layersize, rnn_layernum,
        ...
        self.char_embedding = tf.get_variable("char_embedding", [Char_vocabu_size, embedding_size], dtype=tf.float32,
                                              initializer=tf.random_uniform_initializer(-math.sqrt(3), math.sqrt(3)))
        ...
        with tf.variable_scope("RL_conv", reuse=None):
            ...
            for i in range(int(self.max_steps - 1)):
                with tf.variable_scope("AST_READER" + str(i), reuse=None):
                    (state, self.embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                     em_Rule_Type, nl_conv, em_Rule_List) = self.ast_reader(state, self.embedding_size, step,
                                                                            halting_probability, remainders, n_updates,
                                                                            previous_state, em_Rule_Type, nl_conv,
                                                                            em_Rule_List)
            Decoder = state
            ...
            for i in range(int(self.max_steps - 1)):
                with tf.variable_scope("DECODER", reuse=None):
                    with tf.variable_scope("QUERY_DECODER" + str(i), reuse=None):
                        (f_state, self.embedding_size, step, halting_probability, remainders, n_updates, previous_state,
                         state, nl_conv, em_Tree_Path) = self.query_decoder(f_state, self.embedding_size, step,
                                                                            halting_probability, remainders, n_updates,
                                                                            previous_state, state, nl_conv,
                                                                            em_Tree_Path)
